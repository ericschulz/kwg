...
dm<-read.csv("rbfgreedymean.csv")
dv<-read.csv("rbfgreedyvariance.csv")
...
dp$param<-factor(dp$param, levels=(c(expression(lambda), expression(beta), expression(tau))))
#data frame with everything we need, i.e. min, max, lower nad upper quantile, median, CIs & mean
...
dp<-data.frame(estimate=exp(c(dm$par1, dm$par3)), id=rep(dm$id, 2), param=rep(c("lambda", "tau"), each=nrow(dm)))
#condition and age
dp$condition<-0
dp$age<-0
...
dp<-data.frame(estimate=exp(c(dv$par1, dv$par3)), id=rep(dv$id, 2), param=rep(c("lambda", "tau"), each=nrow(dv)))
#condition and age
dp$condition<-0
dp$age<-0
...
pdf(file = "fullgp.pdf", width = 16, height = 16)
